...
const dom = {
  ...
  cacheDom() {
    ...
    this.closeTaskModalButton =
      this.taskModal.querySelector('.close-task-modal');
    ...
    this.closeEditModalButton =
      this.editModal.querySelector('.close-edit-modal');
    this.submitEditTaskButton =
      this.editModal.querySelector('.submit-edit-task');
    ...
    this.closeProjectModalButton = this.projectModal.querySelector(
      '.close-add-project-modal',
    );
    this.submitAddProjectButton = this.projectModal.querySelector(
      '.submit-add-project',
    );
  ...
  bindEvents() {
    ...
    this.closeTaskModalButton.addEventListener('click', () =>
    ...
    this.projectTitle.addEventListener('input', () =>
  ...
  handleProjectSubmit(e) {
    ...
    newProjectDiv.addEventListener('click', (event) =>
      this.handleProjectRowClick(event),
    );
  ...
  handleEdit(itemId) {
    ...
    const { item } = logic.locateItem(itemId);
  ...
  handleEditSubmit(e) {
    e.preventDefault();
    const { itemId } = this.submitEditTaskButton.dataset;
    const title = this.editFormTitle.value;
    const description = this.editFormDescription.value;
    const dueDate = this.editFormDueDate.value;
    const priority = this.editFormPriority.value;
  ...
  handleTaskSubmit(e) {
    ...
    const title = this.taskFormTitle.value;
    const description = this.taskFormDescription.value;
    const dueDate = this.taskFormDueDate.value;
    const priority = this.taskFormPriority.value;
  ...
  display(project) {
    ...
    this.currentProject.innerText =
      project === 'default'
        ? 'INBOX'
        : project.toUpperCase().split('-').join(' ');
    const displayProject = logic.projects[project];
    for (const itemId in displayProject) {
      if (Object.hasOwn(displayProject, itemId)) {
        const checked = displayProject[itemId].completed;
        const { title } = displayProject[itemId];
        const { description } = displayProject[itemId];
        const { dueDate } = displayProject[itemId];
        const { priority } = displayProject[itemId];
